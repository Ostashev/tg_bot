...
from src.keyboards.inline.create_mail import get_kb_add_film, get_kb_confirm
from src.states.base import CreateMessage, CreateFilm
...
@router.message(CreateFilm.get_text, F.text)
async def set_text_handler_film(message: Message, state: FSMContext):
    await state.update_data(msg_text=message.md_text)
    await message.answer(
        text=(f"Добавить '{message.md_text}'?"),
        reply_markup=get_kb_add_film().as_markup(),
        parse_mode=ParseMode.MARKDOWN
    )
    await state.set_state(CreateFilm.confirm_sender)